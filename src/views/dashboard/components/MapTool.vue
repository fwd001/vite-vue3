<template>
  <div class="absolute top-0 left-0 flex flex-col bg-#fff">
    <span
  ...
  <ClickOutSide
    @click-outside="A.hideContextMenu"
    class="menu-box"
    :style="{ top: `${menuPosition.top}px`, left: `${menuPosition.left}px` }"
  >
    <div class="menu-item" @click="onEditLayerInfo">编辑</div>
    <div v-if="menuPosition.type === 'overlay'" class="menu-item" @click="onEditOverlay">
      修改路径
    </div>
    <div v-if="menuPosition.type === 'marker'" class="menu-item" @click="onMoveLayer">移动</div>
    <div class="menu-item" @click="onDeleteLayer">删除</div>
  </ClickOutSide>
</template>
...
<script setup lang="ts">
  ...
  const A = reactive<Recordable>({
    ...
    showContextMenu(x: number, y: number, type = 'marker') {
  ...
  function eventFn() {
    A.map
      ...
      .on(BM.Draw.Event.DRAWSTOP, function (e) {
        console.log('stop: ,', e.target.stopPropagation);
        console.log('stop: ,', e);
      })
      .on('click', function () {
