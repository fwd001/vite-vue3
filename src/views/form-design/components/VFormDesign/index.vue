...
<script lang="ts" setup>
  ...
  // 子组件实例引用
  const propsPanel = ref<null | IPropsPanel>(null); // 属性面板
  const jsonModal = ref<null | IToolbarMethods>(null); // JSON弹窗
  const importJsonModal = ref<null | IToolbarMethods>(null); // 导入JSON弹窗
  const eFormPreview = ref<null | IToolbarMethods>(null); // 预览弹窗1
  const eFormPreview2 = ref<null | IToolbarMethods>(null); // 预览弹窗2
  const codeModal = ref<null | IToolbarMethods>(null); // 代码弹窗
  ...
  const formConfig = ref<IFormConfig>({
    ...
    activeKey: 1, // 当前属性面板tab
  });

  // 设置表单配置（用于外部导入等场景）
  const setFormConfig = (config: IFormConfig) => {
    // 外部导入时，可能会缺少必要的信息，做容错处理
  ...
  /**
   * 复制表单项，如果表单项为栅格布局，则遍历所有子表单项重新生成key
  ...
  const handleCopy = (
    ...
    /**
     * 遍历表单项配置，如果是复制，则复制一份表单项，如果不是复制，则直接添加到表单项中
     * @param schemas
     */
